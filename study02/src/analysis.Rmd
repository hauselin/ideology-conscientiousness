...
# m1_1 <- glm(share ~ demrep_c * bfi_c, family = binomial, data = dt1)
summary(m1_1)
m1_1c <- coeftest(m1_1, vcovCL(m1_1, ~ responseid + headline_id, NULL, fix = FALSE))
# m1_1c <- coeftest(m1_1, vcovCL(m1_1, ~ responseid, NULL, fix = FALSE))
m1_1c
...
```{r model3}
# Model 3 (all headlines): sharing ~ headline veracity (0/1) * ideology * ( conscientiousness + extraversion + agreeableness + negative emotionality + openness + age + gender + education + liberal-leaning-news [0/1] + conservative-leaning-news [0/1] + attentiveness + thinking styles )
...
# BF
(samplesize <- nobs(m3_1))
m3_1c
compute_bf(m3_1c, samplesize)
bf <- compute_bf(m3_1c, samplesize, "veracity:demrep_c:bfi_c = 0")
summary(bf)
```
